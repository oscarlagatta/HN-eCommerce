...
var accountRegisterModule = angular.module('accountRegister', ['common'])
    .config(function ($routeProvider, $locationProvider) {
        $routeProvider.when(eCommerce.rootPath + 'account/register/step1', { templateUrl: eCommerce.rootPath + 'Templates/RegisterStep1.html', controller: 'AccountRegisterStep1ViewModel' });
        $routeProvider.when(eCommerce.rootPath + 'account/register/step2', { templateUrl: eCommerce.rootPath + 'Templates/RegisterStep2.html', controller: 'AccountRegisterStep2ViewModel' });
        $routeProvider.when(eCommerce.rootPath + 'account/register/step3', { templateUrl: eCommerce.rootPath + 'Templates/RegisterStep3.html', controller: 'AccountRegisterStep3ViewModel' });
        $routeProvider.when(eCommerce.rootPath + 'account/register/confirm', { templateUrl: eCommerce.rootPath + 'Templates/RegisterConfirm.html', controller: 'AccountRegisterConfirmViewModel' });
        $routeProvider.otherwise({ redirectTo: eCommerce.rootPath + 'account/register/step1' });
...
accountRegisterModule.controller("AccountRegisterViewModel", function ($scope, $http, $location, $window, viewModelHelper) {
    ...
    $scope.accountModelStep1 = new eCommerce.AccountRegisterModelStep1();
    $scope.accountModelStep2 = new eCommerce.AccountRegisterModelStep2();
    $scope.accountModelStep3 = new eCommerce.AccountRegisterModelStep3();
...
accountRegisterModule.controller("AccountRegisterStep1ViewModel", function ($scope, $http, $location, viewModelHelper, validator) {
    ...
    var setupRules = function () {
        ...
        //}));
        //accountModelStep1Rules.push(new validator.PropertyRule("PostCode", {
        //    required: { message: "Post code is required" },
        //    pattern: { message: "Post code is in invalid format", params: /^\d{5}$/ }
        //}));
        ...
        if (viewModelHelper.modelIsValid) {
            viewModelHelper.apiPost('api/account/register/validate1', $scope.accountModelStep1,
                function (result) {
                    ...
                    $location.path(eCommerce.rootPath + 'account/register/step2');
                });
        }
        else
            viewModelHelper.modelErrors = $scope.accountModelStep1.errors;
...
accountRegisterModule.controller("AccountRegisterStep2ViewModel", function ($scope, $http, $location, viewModelHelper, validator) {
    ...
    if (!$scope.accountModelStep1.Initialized) {
        ...
        $location.path(eCommerce.rootPath + 'account/register/step1');
    }
        ...
        if (viewModelHelper.modelIsValid) {
            viewModelHelper.apiPost('api/account/register/validate2', $scope.accountModelStep2,
                function (result) {
                    ...
                    $location.path(eCommerce.rootPath + 'account/register/step3');
                });
        }
        else
            viewModelHelper.modelErrors = $scope.accountModelStep2.errors;
        ...
        if (viewModelHelper.modelIsValid) {
            viewModelHelper.apiPost('api/account/register/validate3', $scope.accountModelStep3,
                function (result) {
                    ...
                    $location.path(eCommerce.rootPath + 'account/register/confirm');
                });
        }
        else
            viewModelHelper.modelErrors = $scope.accountModelStep3.errors;
...
accountRegisterModule.controller("AccountRegisterConfirmViewModel", function ($scope, $http, $location, $window, viewModelHelper) {
    ...
    $scope.createAccount = function () {
        ...
        viewModelHelper.apiPost('api/account/register', accountModel,
            function (result) {
                //$location.path(eCommerce.rootPath);
                $window.location.href = eCommerce.rootPath;
            });
    }
});
