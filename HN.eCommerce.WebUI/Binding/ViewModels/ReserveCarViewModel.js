...
var reserveCarModule = angular.module('reserveCar', ['common'])
    .config(function ($routeProvider, $locationProvider) {
        $routeProvider.when(eCommerce.rootPath + 'customer/reservecar', { templateUrl: eCommerce.rootPath + 'Templates/Reserve.html', controller: 'ReserveViewModel' });
        $routeProvider.when(eCommerce.rootPath + 'customer/reservecar/carlist', { templateUrl: eCommerce.rootPath + 'Templates/CarList.html', controller: 'CarListViewModel' });
        $routeProvider.otherwise({ redirectTo: eCommerce.rootPath + 'customer/reservecar' });
...
reserveCarModule.controller("ReserveCarViewModel", function ($scope, $window, viewModelHelper) {
    ...
    $scope.reserveCarModel = new eCommerce.ReserveCarModel();
...
reserveCarModule.controller("ReserveViewModel", function ($scope, $http, $location, viewModelHelper, validator) {
    ...
    $scope.submit = function () {
        if ($scope.reserveCarModel.PickupDate != null && $scope.reserveCarModel.PickupDate != '')
            $scope.reserveCarModel.PickupDate = moment($scope.reserveCarModel.PickupDate).format('DD-MM-YYYY');
        else
            $scope.reserveCarModel.PickupDate = '';

        if ($scope.reserveCarModel.ReturnDate != null && $scope.reserveCarModel.ReturnDate != '')
            $scope.reserveCarModel.ReturnDate = moment($scope.reserveCarModel.ReturnDate).format('DD-MM-YYYY');
        else
        ...
        if (viewModelHelper.modelIsValid) {
            ...
            $location.path(eCommerce.rootPath + 'customer/reservecar/carlist');
        }
        else
            viewModelHelper.modelErrors = $scope.reserveCarModel.errors;
    }
...
reserveCarModule.controller("CarListViewModel", function ($scope, $http, $location, viewModelHelper) {
    ...
    $scope.availableCars = function () {
        if (!$scope.reserveCarModel.initialized) {
            $location.path(eCommerce.rootPath + 'customer/reservecar');
        }
        else {
